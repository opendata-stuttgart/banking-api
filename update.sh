...
#docker run --name banking-nginx --net="host" --volumes-from banking-data -p 80:80 -v `pwd`/nginx.conf:/etc/nginx/nginx.conf -v `pwd`/banking-nginx.conf:/etc/nginx/banking-nginx.conf --restart=always -d nginx
